...
DATA_PATH = Path("data/raw/electionsBR/2018/party_mun_zone.csv")
OUTPUT_PATH = Path("scraping/output/partidos_votos_validos_2018.csv")
    ...
    with DATA_PATH.open("r", encoding="latin1", newline="") as handle:
        ...
        for row in reader:
            if row.get("DS_CARGO") != TARGET_CARGO:
                continue
            if "QT_VOTOS_NOMINAIS_VALIDOS" in row:
                nominal = parse_int(row.get("QT_VOTOS_NOMINAIS_VALIDOS"))
            else:
                nominal = parse_int(row.get("QT_VOTOS_NOMINAIS"))
            if "QT_TOTAL_VOTOS_LEG_VALIDOS" in row:
                legenda = parse_int(row.get("QT_TOTAL_VOTOS_LEG_VALIDOS"))
            else:
                legenda = (
                    parse_int(row.get("QT_VOTOS_LEGENDA_VALIDOS"))
                    + parse_int(row.get("QT_VOTOS_NOMINAIS_CONVR_LEG"))
                    + parse_int(row.get("QT_VOTOS_LEGENDA"))
                )
